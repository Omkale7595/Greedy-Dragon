...
async def main():
    # Global declarations to ensure variables are accessible inside the async loop
    global score, high_score, Velocity, timer, dragon_img, dragon_rect, next_meat_score
    global move_u, move_d, move_l, move_r, pause, score_board_text, timer_text, show_green_box
    global next_golden_meat_score, meat_active, golden_meat_active, game_mode, running
    global dragon_left, dragon_right, dragon_up, dragon_down, coin_img, coin_rect
    global meat_img, meat_img_rect, golden_meat, golden_meat_rect, pause_img, pause_img_rect
    global return_img, return_img_rect, home_img, home_img_rect, bg_music, click, coin_pickup
    ...
    def reset():
        ...
        next_meat_score = 0
        next_golden_meat_score = 5
        score_board_text = score_board.render(f"Score:{score}", True, (34, 139, 34))
        timer_text = score_board.render(f"Time:{timer}", True, (0, 0, 0))
    ...
    golden_meat, golden_meat_rect = image.image("dragon_game/golden_meat.png", (400, 300))
    next_meat_score = 0
    meat_active = False
    next_golden_meat_score = 5
    golden_meat_active = False
    ...
    new_font = pygame.font.Font('dragon_game/Nangorians.ttf', 108)
    title_text = new_font.render('Greedy Dragon', True, Red)
    title_text_rect = title_text.get_rect()
    title_text_rect.center = (550, 800)
    ...
    while running:
        if (game_mode == 'Menu'):
            dis_s.fill(White)
            dis_s.blit(title_text, title_text_rect)
            ...
            if (move_l and dragon_rect.centerx > 0 and not pause):
                ...
                if (dragon_rect.left < 0): dragon_rect.left = 0
            if (move_r and dragon_rect.x < 1080 and not pause):
                dragon_rect.x += Velocity
                if (dragon_rect.right > 1080): dragon_rect.right = 1080
            if (move_u and dragon_rect.y > 0 and not pause):
                dragon_rect.y -= Velocity
                if (dragon_rect.top < 0): dragon_rect.top = 0
            if (move_d and dragon_rect.y < 1600 and not pause):
                dragon_rect.y += Velocity
                if (dragon_rect.bottom > 1600): dragon_rect.bottom = 1600
                
            if (dragon_rect.colliderect(coin_rect) or coin_rect.colliderect(score_board_text_rect) or coin_rect.colliderect(timer_text_rect) or coin_rect.colliderect(pause_img_rect) or coin_rect.colliderect(golden_meat_rect)):
                coin_rect.x, coin_rect.y = random.randint(0, 1000), random.randint(0, 1600)
                coin_pickup.play()
                if score < 20: Velocity += 0.2
                elif (score < 50): Velocity += 0.1
                else: Velocity += 0.05
            ...
            if (score >= next_meat_score and not meat_active and score >= 0):
                meat_active = True
            if (score > next_golden_meat_score and not golden_meat_active):
                golden_meat_active = True
            if (timer <= 10):
            ...
            if meat_active:
                ...
                if dragon_rect.colliderect(meat_img_rect):
                    ...
                    meat_pickup.play()
                    if (score < 50): timer += 8
                    elif (score < 100): timer += 6
                    elif (score >= 100): timer += 4
            
            if golden_meat_active:
                dis_s.blit(golden_meat, golden_meat_rect)
                if dragon_rect.colliderect(golden_meat_rect) and Velocity > 2:
                    golden_meat_active = False
                    next_golden_meat_score = score + 10
                    meat_pickup.set_volume(random.uniform(0.5, 0.8))
                    meat_pickup.play()
                    timer += 10
                    Velocity -= 2
                    
            if (dragon_rect.colliderect(meat_img_rect) or meat_img_rect.colliderect(score_board_text_rect) or meat_img_rect.colliderect(timer_text_rect) or meat_img_rect.colliderect(pause_img_rect) or meat_img_rect.colliderect(golden_meat_rect)):
                meat_img_rect.x, meat_img_rect.y = random.randint(0, 1000), random.randint(0, 1600)
            if (dragon_rect.colliderect(golden_meat_rect) or golden_meat_rect.colliderect(score_board_text_rect) or golden_meat_rect.colliderect(timer_text_rect) or golden_meat_rect.colliderect(pause_img_rect) or meat_img_rect.colliderect(golden_meat_rect)):
                golden_meat_rect.x, golden_meat_rect.y = random.randint(0, 1000), random.randint(0, 1600)
        ...
        # 3. THE WEB MAGIC - Lets the browser breathe
